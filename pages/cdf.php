...
<body>
    ...
    <main>
        <h1 style="margin-bottom:40px">En Sélections</h1>
...
foreach ($donnees as $row) {
    // Utiliser la fonction strpos() pour vérifier si la sous-chaîne "EURO U" est présente au début de la valeur de 'CHAMPIONNAT'
    if (strpos($row['CHAMPIONNAT'], 'CDF') === 0) {
        $euroU20RowCount++;
    }
}
?>
...
foreach ($donnees as $row) {
    // Vérifier si le championnat est Euro U20
    if (strpos($row['CHAMPIONNAT'], 'CDF') !== false) {
